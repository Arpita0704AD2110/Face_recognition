...
def show_fullscreen_image(image, window_name="Result"):
    ...
    screen_res = 1920, 1080  # or use pyautogui.size() for dynamic
...
for filename in os.listdir(test_folder):
    ...
    for encoding, (top, right, bottom, left) in zip(encodings, boxes):
        ...
        name = name.lower()  # Lowercase for display

        # Extract ground truth from filename (e.g., Alice_1.jpg → Alice)
        actual_name = filename.split('_')[0].lower()

        if name != "unknown":
            predicted_labels.append(name)
            true_labels.append(actual_name)
